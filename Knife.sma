...
#define NUM_KNIVES sizeof(KnifeNames)
...
new player_knife[MAX_PLAYERS + 1];
new g_vault;
...
public Knife(id) {
    ...
    for (new i = 0; i < NUM_KNIVES; i++) {
        if (FlagsKnife[i] <= 0 || (FlagsKnife[i] > 0 && get_user_flags(id) & FlagsKnife[i])) {
            static itemName[60];
            formatex(itemName, 59, "%s%s", FlagsKnife[i] > 0 ? "\r[PREMIUM]\w " : "", KnifeNames[i]);
            menu_additem(menu, itemName, "", FlagsKnife[i], -1);
        }
    }
...
public Callback(id, menu, item) {
    ...
    if (FlagsKnife[item] <= 0 || (FlagsKnife[item] > 0 && get_user_flags(id) & FlagsKnife[item])) {
        player_knife[id] = item;
        ColorChat(id, GREEN, "[Knife Skin Changer]^x01 %L ^x04%s", LANG_PLAYER, "KNIFE_SELECT", KnifeNames[player_knife[id]]);
        Set_Model(id);
        SaveKnife(id);
    }
